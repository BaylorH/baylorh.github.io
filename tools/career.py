...
def career_page(projects,brand_art):
 ...
 def product_list(items):
  return '<ul class="career-products">'+''.join('<li><strong>'+e(x['name'])+'.</strong> '+e(x['text'])+'</li>' for x in items)+'</ul>'
 def role(v):
  body='<p class="career-role-intro">'+e(v['intro'])+'</p>' if v.get('intro') else ''
  if v.get('groups'):
   body+='<div class="career-product-groups">'+''.join('<section class="career-product-group"><h4>'+e(g['title'])+'</h4><p class="career-group-context">'+e(g['context'])+'</p>'+product_list(g['projects'])+'</section>' for g in v['groups'])+'</div>'
  elif v.get('products'):body+=product_list(v['products'])
  else:body+=bullets(v['bullets'])
  return '<div class="career-role"><div class="career-role-heading"><h3>'+e(v['role'])+'</h3><span>'+e(v['period'])+'</span></div>'+body+'</div>'
 entries=''
 for i,x in enumerate(c['experience']):
  ...
  if x.get('clients'):
   entries+='<div class="career-clients"><p class="eyebrow">Selected client work</p><div class="career-client-grid">'+''.join(f'<a href="{pid}.html" aria-label="Explore {e(by[pid]["name"])}">'+brand_art(by[pid],'career-client-logo',True)+'</a>' for pid in x['clients'])+'</div></div>'
